...
import { applicationjson, CARBORN_SITE } from "../../../lib/api";
...
// 정비업체, 업체주소
const StyleTableCarInspectorShopDiv = styled.div`
  width: 24vw;
  height: 3vh;
  margin-bottom: 1vh;
...
const StyleTableInspectorModifyButtonDiv = styled.div`
  width: 35vw;
  height: 20vh;
  display: flex;
...
export const StyleInspectorBookModifyContainer = styled.div`
  height: 20vh;
...
const MyInspectorBookDetail = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios({
          method: "GET",
          url: `${CARBORN_SITE}/api/user/inspect/book/${carId}`,
          headers: {
        ...
        switch (modifiedContent.bookStatus) {
          case 0:
            ...
            break;
          case 1:
            modifiedContent.bookStatusString = "검수 완료";
            modifiedContent.bookStatusNumber = 1;
            break;
          case 2:
            modifiedContent.bookStatusString = "검수 취소";
  ...
  // 예약 변경 모달 
  const [isInspectorBookModify, setIsInspectorBookModify] = useState<boolean>(false);
  const handleInspectorBookModify = () => {
    setIsInspectorBookModify(!isInspectorBookModify);
  };
  
  // 예약 취소 모달
  const [isInspectorBookDelete, setIsInspectorBookDelete] = useState<boolean>(false);
  const handleInspectorBookDelete = () => {
    setIsInspectorBookDelete(!isInspectorBookDelete);
  };
      ...
      <StyleMyInspectorDetailContainerDiv>
        ...
        <StyledTableInspectorContainer>
          <StyleTableDivInspectorDetail>
            ...
            {data && (
              <>
                <StyleTableCarInspectorShopDiv>
                  {/* 업체명 */}
                  <div>업체명</div>
                  {data.inspectorAccountName && (
                    <div>{data.inspectorAccountName}</div>
                  )}
                </StyleTableCarInspectorShopDiv>
                <StyleTableCarInspectorShopDiv>
                  {/* 업체주소 */}
            ...
            <StyleTableInspectorModifyButtonDiv>
              <button onClick={handleInspectorBookModify}>예약 변경하기</button>
              <button onClick={handleInspectorBookDelete}>예약 취소하기</button>
            </StyleTableInspectorModifyButtonDiv>
            <BookModifyDialogSlide 
              open={isInspectorBookModify} 
              onClose={handleInspectorBookModify}
              data={data}
            />
            <BookDeleteDialogSlide 
              open={isInspectorBookDelete} 
              onClose={handleInspectorBookDelete}
              data={data}
            />
          </StyleTableDivInspectorDetail>
        </StyledTableInspectorContainer>
