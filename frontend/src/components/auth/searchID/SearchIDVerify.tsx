...
const SearchIDVerify = ({ setinputObj, inputObj }:SearchIDVerifyProps) => {
  ...
  const openModal = () => {
    if (inputObj.phonenumber.length <= 10 && inputObj.phonenumber.length <= 11) {
  ...
  return (
    <StyleLoginInputDiv>
      ...
      {/* 모달 */}
      <SignUpUserPhoneNumberModal open={isModalOpen} onClose={closeModal} phoneNumber={inputObj.phonenumber} setIsValid={setIsValid} isValid={isValid}/>
    </StyleLoginInputDiv>
  );
};
