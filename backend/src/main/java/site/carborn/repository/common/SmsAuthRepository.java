...
public interface SmsAuthRepository extends JpaRepository<SmsAuth, Integer> {
    @Query(value = "SELECT * FROM MWS_SMS_AUTH WHERE (TIMESTAMPDIFF(MINUTE, NOW(), EXP_DT) BETWEEN 0 AND 3) and PHONE_NM = :phoneNm and STATUS = 1 ORDER BY EXP_DT DESC LIMIT 1;", nativeQuery = true)
